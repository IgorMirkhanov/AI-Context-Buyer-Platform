...
export type AnthropicOptimizationLlmOptions = AnthropicClientOptions;

export class AnthropicOptimizationLlm implements OptimizationWriter {
  private readonly heuristic = new HeuristicOptimizationLlm();
  private readonly options: AnthropicOptimizationLlmOptions;

  constructor(options: AnthropicOptimizationLlmOptions) {
    ...
    try {
      ...
      const insights =
        parsed?.insights?.length === facts.length
          ? parsed.insights
          : facts.map((fact, index) => parsed?.insights?.[index] ?? `Рекомендация: ${fact}`);
      return {
        insights,
        prompt,
