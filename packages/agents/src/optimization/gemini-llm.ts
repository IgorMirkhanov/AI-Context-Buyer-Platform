...
export type GeminiOptimizationLlmOptions = {
  apiKey: string;
  model?: string;
...
export class GeminiOptimizationLlm implements OptimizationWriter {
  private readonly heuristic = new HeuristicOptimizationLlm();
  private readonly options: GeminiOptimizationLlmOptions;

  constructor(options: GeminiOptimizationLlmOptions) {
  ...
  async wrap(facts: string[]): Promise<OptimizationWrapResult> {
    ...
    try {
      ...
      const parsed = parseJsonFromGeminiLlm<{ insights?: string[] }>(
        result.text,
      );
      const insights =
